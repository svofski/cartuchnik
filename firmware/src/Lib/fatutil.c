...
void InitializeFAT12(DISKIMAGE *DiskImagePtr)
{
	...
	//memcpy(DiskImagePtr->BootSector.BS_VolLab, "BOBLABEL   ", 11);
	
	static const uint8_t fatstart[] = {0xf8, 0xff, 0xff};

	memset(&DiskImagePtr->Fat[0].FATByte[0], 0, sizeof(DiskImagePtr->Fat[0]));
	memcpy(&DiskImagePtr->Fat[0].FATByte[0], &fatstart, 3);

	memset(&DiskImagePtr->Fat[1].FATByte[0], 0, sizeof(DiskImagePtr->Fat[0]));
	memcpy(&DiskImagePtr->Fat[1].FATByte[0], &fatstart, 3);

#if 0
	// Not sure what the authors meant by this, it creates a bunch of orphaned FAT entries
	...
	xprintf("FAT dump\n");
	for (i = 0; i < 256; i++) { 
		xprintf("%x ", DiskImagePtr->Fat[0].FATByte[i]);
	}
	xprintf("\n");
#endif
}
...
void CreateDiskImage(DISKIMAGE *DiskImagePtr)
{
	...
	DiskImagePtr->BootSector.BS_VolID = 0x1337c0d3;
	// Set the volume label
	strncpy((char *) DiskImagePtr->DirectoryEntries[0].Name, "VECTREXA", 8);
	strncpy((char *) DiskImagePtr->DirectoryEntries[0].Extension, "DOR", 3);
